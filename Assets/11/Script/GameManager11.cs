...
public class GameManager11 : MonoBehaviour
{
    ...
    public GameObject nextSceneButton;  // 次のステージへ進むボタン
    ...
    void Start()
    {
        ...
        nextSceneButton.SetActive(false);   // 次のステージを進むボタンを非表示へ
    ...
    public void Replay()
    {
        int sceneIndex = SceneManager.GetActiveScene().buildIndex;  // 現在のシーンのindex値を取得
        SceneManager.LoadScene(sceneIndex); // 現在読み込んでいるSceneをindex値でロード（＝シーンの再読み込み）
    }
    ...
    public void NextScene()
    {
        ...
        if (j - i > 0)  // 登録されている数より大きいindexのシーンでない?(Yes)
        {
            SceneManager.LoadScene(i);  // 次のシーンへ進む
        }
    }
    ...
    void Update()
    {
        if (inGame) // ゲーム中?(Yes)
        {
            ...
            if (targetObj == null)  
            {
                ...
                nextSceneButton.SetActive(true);        // 次のシーンへ進むボタンを表示
