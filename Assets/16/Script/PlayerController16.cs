...
public class PlayerController16 : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Goal") // 接触したオブジェクトのタグが「Goal」?(Yes)
        {
            ...
            rB.AddForce(-rbVelo.x * 0.8f, 0, -rbVelo.z * 0.8f, ForceMode.Impulse);  // 力を与える
        ...
        {
            rB.AddForce(0, jumpForce, 0, ForceMode.Impulse);    // 力を与える
        }
    }
    ...
    {
        if (other.gameObject.tag == "Turbo")    // タグが「Turbo」?(Yes)
        {
